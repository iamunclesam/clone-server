...
import { findRuntimeAction } from "@clone/integration-framework";
import {
...
// Re-export AuthorityResult for consumers
...
type SafetyCheck = SafetyViolation | SafetyPassed;
...
function checkSafetyRules(
  ...
  if (FINANCIAL_ACTIONS.has(actionId)) {
    const hasFinancialAuthority =
      context.authority.level === "FOUNDER" ||
...
export class AuthorityGate {
  ...
  evaluate(input: {
    ...
    const actionDef = findRuntimeAction(actionId);
    ...
    if (needsApproval) {
      return {
        decision: "APPROVAL",
        reason: `"${actionId}" requires approval before execution. ${

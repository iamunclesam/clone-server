...
exports.DelegationEngine = void 0;
...
class DelegationEngine {
    ...
    async validate(input) {
        ...
        const fromClone = await database_1.prisma.aIEmployee.findFirst({
        ...
        const toClone = await database_1.prisma.aIEmployee.findFirst({
        ...
        if (input.requiredIntegrations && input.requiredIntegrations.length > 0) {
            ...
            const targetPermProviders = new Set((toClone.permissions || []).map((p) => {
    ...
    async delegate(input) {
        ...
        try {
            ...
            // ── Create Commitment ──────────────────────────────────────────────────
            const commitment = await database_1.prisma.runtimeCommitment.create({
                data: {
                    companyId: input.companyId,
            ...
            if (input.sourceExecutionId) {
                const exec = await database_1.prisma.runtimeExecution.findFirst({
                    where: { id: input.sourceExecutionId },
                });
                if (exec) {
                    const stepLog = exec.stepLog || [];
                    stepLog.push({
     ...
     */
    async chooseBestTarget(compiled, companyId) {
...
exports.DelegationEngine = DelegationEngine;

...
export class CommitmentMonitor {
   ...
   */
  async scan(companyId: string): Promise<CommitmentScanResult> {
    const now = new Date();
    const result: CommitmentScanResult = {
      checked: 0,
      markedOverdue: 0,
      reminded: 0,
      escalated: 0,
    ...
    for (const commitment of commitments) {
      ...
      if (msUntilDue <= 0 && commitment.status !== "OVERDUE") {
        await prisma.runtimeCommitment.update({
   ...
   */
  private async escalateCommitment(
    commitment: any,
    companyId: string,
    now: Date
  ): Promise<void> {
    ...
    await prisma.runtimeEscalation.create({
      data: {
        companyId,
   ...
   */
  async complete(commitmentId: string, companyId: string): Promise<void> {
    const commitment = await prisma.runtimeCommitment.findFirst({
  ...
  async createFromExecution(input: {
    ...
    return prisma.runtimeCommitment.create({
      data: {
   ...
   */
  async getOpenCommitments(cloneId: string, companyId: string): Promise<any[]> {
    return prisma.runtimeCommitment.findMany({
      where: {
        cloneId,
        companyId,

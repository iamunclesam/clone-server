...
exports.RUNTIME_INTEGRATIONS = void 0;
exports.getRuntimeManifest = getRuntimeManifest;
exports.listRuntimeActions = listRuntimeActions;
exports.listRuntimeEvents = listRuntimeEvents;
exports.findRuntimeEvent = findRuntimeEvent;
exports.findRuntimeAction = findRuntimeAction;
exports.getAllProviders = getAllProviders;
exports.findRelevantEvents = findRelevantEvents;
exports.findAvailableActions = findAvailableActions;
exports.getCriticalEvents = getCriticalEvents;
// ─── Registry ─────────────────────────────────────────────────────────────────
exports.RUNTIME_INTEGRATIONS = [
    // ── GitHub ──────────────────────────────────────────────────────────────────
    {
        provider: "github",
        name: "GitHub",
...
// ─── Lookup helpers ───────────────────────────────────────────────────────────
function getRuntimeManifest(provider) {
    return exports.RUNTIME_INTEGRATIONS.find((m) => m.provider === provider);
}
function listRuntimeActions(provider) {
    return getRuntimeManifest(provider)?.actions ?? [];
}
function listRuntimeEvents(provider) {
    return getRuntimeManifest(provider)?.events ?? [];
}
function findRuntimeEvent(eventId) {
    for (const m of exports.RUNTIME_INTEGRATIONS) {
...
function findRuntimeAction(actionId) {
    for (const m of exports.RUNTIME_INTEGRATIONS) {
...
function getAllProviders() {
    return exports.RUNTIME_INTEGRATIONS.map((m) => m.provider);
}
/**
 ...
 */
function findRelevantEvents(providers, identityText) {
    const results = [];
    for (const provider of providers) {
        for (const event of listRuntimeEvents(provider)) {
            if (event.relevance.length === 0 ||
 ...
 */
function findAvailableActions(providers) {
    const results = [];
    for (const provider of providers) {
...
function getCriticalEvents() {
    return exports.RUNTIME_INTEGRATIONS.flatMap((m) => m.events.filter((e) => e.severity === "CRITICAL"));
}

...
exports.GitHubIntegrationProvider = void 0;
...
class GitHubIntegrationProvider {
    ...
    async getCapabilities(_connectionId) {
        return [
            {
    ...
    async executeTool(input) {
        ...
        if (input.toolName === "github.read_repository") {
            ...
            const files = Array.isArray(body)
                ? body.map((f) => f.path || f.name).filter(Boolean)
        ...
        if (input.toolName === "github.create_branch") {
            ...
            if (!sha)
                return { success: false, data: null, error: `Base branch ${baseBranch} has no SHA.` };
...
exports.GitHubIntegrationProvider = GitHubIntegrationProvider;

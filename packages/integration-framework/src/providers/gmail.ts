...
interface ParsedOAuthCreds {
  clientId: string;
  clientSecret: string;
  source: "env" | "client_secret_json" | "service_account_json";
  serviceAccountEmail?: string;
  privateKey?: string;
  userImpersonationEmail?: string;
}

let _cachedParsedCreds: ParsedOAuthCreds | null = null;

function parseGoogleOAuthCreds(): ParsedOAuthCreds {
  if (_cachedParsedCreds) return _cachedParsedCreds;
...
function classifyGoogleTokenError(bodyText: string): string {
  let hint = "";
  try {
    const j = JSON.parse(bodyText);
    const err = (j.error || "") as string;
    const desc = (j.error_description || "") as string;
    ...
    if (err) hint = ` (${err}: ${desc.replace(/[<>\n\r]/g, " ").slice(0, 200)})${hint}`;
  } catch {}
  return hint;
}

async function getAccessTokenForServiceAccount(creds: ParsedOAuthCreds, scopes: string[]): Promise<string | null> {
  if (!creds.privateKey || !creds.serviceAccountEmail) return null;
  try {
    ...
    if (creds.userImpersonationEmail) claimSet.sub = creds.userImpersonationEmail;
    const encodeB64 = (o: any) => Buffer.from(JSON.stringify(o)).toString("base64url");
...
async function refreshGoogleAccessToken(refreshToken: string): Promise<string | null> {
  ...
  if (!creds.clientId || (!creds.clientSecret && creds.source === "env") || !refreshToken) return null;
...
export class GmailIntegrationProvider implements IntegrationProvider {
  ...
  async getAuthorizationUrl(input: { companyId: string; userId: string; redirectUri: string; state: string }): Promise<string> {
    const creds = parseGoogleOAuthCreds();
    if (!creds.clientId) throw new Error("GOOGLE_CLIENT_ID or GOOGLE_OAUTH_CLIENT_JSON is not configured");
    if (creds.source === "service_account_json") {
  ...
  async executeTool(input: { toolName: string; arguments: Record<string, unknown> }) {
    const initialAccessToken = (input.arguments?.accessToken as string) || "";
    const refreshToken = (input.arguments?.refreshToken as string) || "";
    let accessToken = initialAccessToken;
    let freshToken: string | undefined;

    const getUsableToken = async (): Promise<string | null> => {
      const creds = parseGoogleOAuthCreds();
      if (creds.source === "service_account_json") {
        const t = await getAccessTokenForServiceAccount(creds, ["https://mail.google.com/", "https://www.googleapis.com/auth/userinfo.email"]);
        if (t) { accessToken = t; freshToken = t; return t; }
      }
    ...
    const handle401 = async (res: Response): Promise<Response | null> => {
      if (res.status !== 401 || !refreshToken) return null;
      const t = await refreshGoogleAccessToken(refreshToken);
      if (!t) return null;
      accessToken = t; freshToken = t;
      const orig = (res as any)._request as any;
      if (!orig) return null;
      try {
        return fetch(orig.url as string, {
          method: orig.method,
          headers: { ...(orig.headers as Record<string, string>), Authorization: `Bearer ${t}` },
          body: orig.body as string,
        });
      } catch { return null; }
    };
    ...
    if (input.toolName === "gmail.read_message") {
      ...
      if (!token) return { success: false, data: null, error: "No Gmail access token available. Reconnect Gmail in integrations settings." };
      try {
        const q = (input.arguments?.query as string) || "";
        const maxR = Math.max(1, Math.min(20, (input.arguments?.maxResults as number) || 5));
        const listUrl = `https://gmail.googleapis.com/gmail/v1/users/me/messages?maxResults=${maxR}${q ? `&q=${encodeURIComponent(q)}` : ""}`;
        const reqInit: RequestInit = { method: "GET", headers: { Authorization: `Bearer ${token}` } };
        let listRes = await fetch(listUrl, reqInit);
        (listRes as any)._request = { ...reqInit, url: listUrl };
        if (listRes.status === 401) {
          const retried = await handle401(listRes);
          if (retried) listRes = retried;
        }
        if (listRes.ok) {
          const listData: { messages?: { id: string }[] } = await listRes.json();
          const realMessages: Array<{ id: string; snippet: string; from: string; subject: string; date: string }> = [];
          if (listData.messages && listData.messages.length > 0) {
            for (const msgRef of listData.messages.slice(0, maxR)) {
              ...
              const mReq: RequestInit = { method: "GET", headers: { Authorization: `Bearer ${accessToken}` } };
              let msgRes = await fetch(mUrl, mReq);
              (msgRes as any)._request = { ...mReq, url: mUrl };
              if (msgRes.status === 401) {
                const retried = await handle401(msgRes);
                if (retried) msgRes = retried;
              }
              if (msgRes.ok) {
    ...
    if (input.toolName === "gmail.draft_message") {
      ...
      if (!token) return { success: false, data: null, error: "No Gmail access token available. Reconnect Gmail in integrations settings." };
      try {
        const { to, subject, body } = input.arguments as { to: string; subject: string; body: string };
        if (!to || !subject) return { success: false, data: null, error: "gmail.draft_message requires 'to' and 'subject' fields" };
        ...
        (res as any)._request = { ...reqInit, url };
        if (res.status === 401) {
          const retried = await handle401(res);
          if (retried) res = retried;
        }
        if (res.ok) {
          const draftData: any = await res.json();
    ...
    if (input.toolName === "gmail.send_message") {
      ...
      const buildRaw = (to: string, subject: string, body: string) => {
        const rawEmail = [
          `To: ${to}`,
          `Subject: ${subject}`,
      ...
      const sendWithToken = async (t: string) => {
        const { to, subject, body } = input.arguments as { to: string; subject: string; body: string };
        if (!to) throw new Error("gmail.send_message requires a 'to' recipient email");
      ...
      try {
        ...
        if (res.ok) {
          const sendData: any = await res.json();
